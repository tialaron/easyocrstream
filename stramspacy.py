...
st.subheader('Коррекция документа')
st.write('Теперь давайте рассмотрим как влияют различные параметры на точность распознавания.')
value_sli = st.sidebar.slider('Коррекция яркости', 0.0, 100.0, 50.0)
value_color = st.sidebar.slider('Цветность', 0, 100, 100)
value_gaus = st.sidebar.slider('Фильтр Гаусса', 0, 10, 0)

col3,col4 = st.columns(2)
with col3:
    st.write('Яркость', value_sli)
    st.write('Цветность', value_color)
    st.write('Фильтр Гаусса', value_gaus)

    image111 = Image.open('out111.jpg')
    enhancer = ImageEnhance.Brightness(image111)
    factor = 2 * value_sli / 100  # фактор изменения
    im_output = enhancer.enhance(factor)
    enhancer2 = ImageEnhance.Color(im_output)
    factor2 = value_color/100
    im_output = enhancer2.enhance(factor2)

    im_output = im_output.filter(ImageFilter.GaussianBlur(radius=value_gaus))


    file_path = path_pict + option1 + '_edit.jpg'
